  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        const response = await axios.post(`${BACKEND_HOST}/activities`, data, {
          headers: { 'Content-Type': 'application/json' }
        });
        if (response.data && response.data.code === 200 && Array.isArray(response.data.data)) {
          setNews(response.data.data);
        } else {
          setError('Không lấy được danh sách tin tức');
      ...
      {loading ? (
        ...
        <div className="space-y-8">
            ...
            <article className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="md:flex">
                <div className="md:w-1/2">
                  <img
                    src={getFileUrl(accessToken, news[0].image_id, '/images/400x400.svg')}
                    alt={news[0].tieu_de}
                ...
                <div className="md:w-1/2 p-8 lg:p-12">
                  <div className="flex items-center space-x-4 text-sm text-gray-500 mb-4">
                    <div className="flex items-center space-x-1">
                      ...
                      <span>{new Date(news[0].ngay_bh).toLocaleDateString('vi-VN')}</span>
                    </div>
                    {/* <div className="flex items-center space-x-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {news.slice(1).map((newItem) => (
              <article key={newItem.id} className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group">
                <div className="relative overflow-hidden">
                  <img
                    src={getFileUrl(accessToken, newItem.image_id, '/images/400x400.svg')}
                    alt={newItem.tieu_de}
                ...
                <div className="p-6">
                  <div className="flex items-center space-x-4 text-sm text-gray-500 mb-3">
                    <div className="flex items-center space-x-1">
                      <Calendar className="h-4 w-4" />
                      <span>{new Date(newItem.ngay_bh).toLocaleDateString('vi-VN')}</span>
                    </div>
                    {/* <div className="flex items-center space-x-1">
                      <User className="h-4 w-4" />
